...
function Navbar() {
  ...
  useEffect(() => {
    if (watch("searchQuery") !== queryTrack) {
      router.push(`/blog?searchQuery=${watch("searchQuery")}`);
      setqueryTrack(watch("searchQuery"));
    }
  }, [watch("searchQuery")]);
  ...
  return (
    <nav className="backdrop-blur-sm fixed z-[1000] h-[70px] px-[30px] items-center flex flex-col lg:flex-row justify-between w-full">
      <a href="/" className="text-[#fc6719] font-[700] text-[40px]">
